"""Test dell'avversario IA via API (parsing della risposta del modello), senza rete."""
...
def test_match_move_chess_by_uci():
    ...
    move = api_ai._match_move(game, legal, "Penso che la mossa migliore sia e2e4, decisa.")
...
def test_match_move_none_when_absent():
    ...
    state = game.initial_state()
    legal = game.legal_moves(state)
    assert api_ai._match_move(game, legal, "non ne ho idea") is None
...
def test_match_move_tictactoe_index():
    ...
    state = game.initial_state()
    legal = game.legal_moves(state)
    move = api_ai._match_move(game, legal, "Gioco nella casella 4 (centro).")
...
def test_match_move_draughts_path():
    ...
    move = api_ai._match_move(game, legal, f"La mia mossa: {target}")
